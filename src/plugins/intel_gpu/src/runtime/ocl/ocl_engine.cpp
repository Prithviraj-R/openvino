...
memory_ptr ocl_engine::create_mmap_hostbuffer(const void* mmapped_address, size_t data_size, allocation_type _allocation_type, const layout output_layout) {
    ...
    auto tracker = std::make_shared<MemoryTracker>(this,
                                                   const_cast<void*>(mmapped_address),  // Track the original tensor base; the OCL buffer uses a page-aligned base.
                                                   data_size,
                                                   _allocation_type);
...
#endif
    cl::Buffer buffer(get_cl_context(), flags, aligned_data_size, mmap_aligned_address, &err);
    OPENVINO_ASSERT(err == CL_SUCCESS, "clCreateBuffer with CL_MEM_USE_HOST_PTR failed with error code: " + std::to_string(err));
